"""
Phase 01 — Cayley-graph sparse attention vs random sparse attention (tiny Shakespeare)

Headline question: at an equal sparse-attention edge budget, does a *deterministic*
SL(2,Z_n) Cayley-expander connectivity (arm B) reach validation loss as close to full
causal attention (arm A) as a *random* BigBird-style connectivity (arm C) — and with
...
Arms (only the attention mask differs; every other hyperparameter is held identical):
  ...
  B — Cayley:      local window(w=64) + 4 Cayley long-range     (proposal, 5 seeds)
  C — Random:      local window(w=64) + 4 random long-range     (BigBird control, 5 seeds)
  D — Pure Cayley: 4 Cayley long-range, NO window               (ablation, 3 seeds)
...
Long-range budget is matched at 4 edges/token (pre-causal) for B, C, D.

Run:  ~/Code/HRS/.venv/bin/python experiments/phase01_cayley_vs_random_sparse.py
Outputs: results/phase01/phase01.json, results/phase01/spectral_gaps.png

The script is resumable: completed (arm, seed) runs already present in phase01.json
...
RESULT_DIR = os.path.join(REPO, "results", "phase01")
JSON_PATH  = os.path.join(RESULT_DIR, "phase01.json")
...
BLOCK_SIZE   = 336      # = |SL(2,Z_7)| : one token position per Cayley node, no padding
N_LAYER      = 8        # >= Cayley diameter (7) so long-range edges can compose
N_HEAD       = 6
N_EMBD       = 384
DROPOUT      = 0.2
WINDOW       = 64       # local causal band width
LONGRANGE_K  = 4        # long-range edges/token (matches Cayley 4-regularity)
CAYLEY_N     = 7
...
ARMS = {
    ...
    "B_cayley":      {"seeds": SEEDS_5, "window": True,  "longrange": "cayley",
                      "desc": "local window + 4 Cayley long-range"},
    "C_random":      {"seeds": SEEDS_5, "window": True,  "longrange": "random",
                      "desc": "local window + 4 random long-range (BigBird control)"},
    "D_pure_cayley": {"seeds": SEEDS_3, "window": False, "longrange": "cayley",
                      "desc": "4 Cayley long-range, no window"},
...
def cayley_adjacency(n):
    ...
    gens = [(1, 1, 0, 1), (1, (n-1), 0, 1), (1, 0, 1, 1), (1, 0, (n-1), 1)]
    N = len(elems)
...
def main():
    ...
    assert N == BLOCK_SIZE and degs == [4], (N, degs)
    print(f"[cayley] N={N} regular={degs} (verified 336 nodes, 4-regular)")
    ...
    res["config"] = {
        ...
        "cayley_diameter": 7, "arms": {k: v["desc"] for k, v in ARMS.items()},
    }
    save_results(res)
