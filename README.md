...
| [07](results/phase07/phase07_README.md) | Practical viability: does a fixed **100-edge** graph (~7–13× cheaper than full attention) match full attention on the D=256 needle the sparse Cayley graph missed, once the edges are *spread* to keep every route ≤2 hops? Sharp control: a **bunched** width-100 window (3 hops) at the *same* budget. Reach guard proves 2-hop spread vs 3-hop window, no direct edge. | **Viability holds — and spreading edges buys ~7× training *speed*, not reach.** `B_expander100`/`B_random100` (2-hop spread) match full attention (0.975/0.988 vs 0.989), all 4 seeds, at ~13× edge saving — construction null a **5th** time. But the equal-budget bunched window did **not** land in the P06 lottery: it solved **4/4 too, just ~7× slower** (median 12,250 vs 1,750 steps). So P06's "3-hop lottery" was a *single-path* effect — a redundant 3-hop window is reliable-but-costly. **Depth sets optimization cost; path multiplicity sets reliability.** Real-text transfer → phase 08. |
